                    ...
                    @foreach ($data as $data)
                        <div class="col-lg-4 menu-item">
                            <a href="assets/img/menu/menu-item-1.png" class="glightbox"><img
                                    src="food_img/{{ $data->image }}" class="menu-img img-fluid" alt=""></a>
                            <h4>{{ $data->title }}</h4>
                            <p class="ingredients">
                                {{ $data->detail }}
                            </p>
                            ...
                            @php
                                $avgRating = $data->ratings->avg('rating');
                            @endphp
                            @if ($avgRating)
                                <p style="color: gold;">★ {{ number_format($avgRating, 1) }} / 5</p>
                            @else
                            ...
                            <form action="{{ url('add_cart', $data->id) }}" method="post">
                                ...
                                <input class="btn btn-info" type="submit" value="Add To Cart"
                                    style="background-color: #D53535; color: white; border: 2px solid black;">
                            </form>
                        </div><!-- Menu Item -->
                    @endforeach()
                </div>
